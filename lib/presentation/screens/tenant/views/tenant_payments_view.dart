...
class TenantPaymentsView extends ConsumerWidget {
        ...
        : Column(
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Text('All Transactions', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold)),
                    const Spacer(),
          ...
          child: Column(
            children: [
              ...
              Row(
                ...
                children: [
                  _buildStatItem('Transactions', '${payments.length}', Colors.white),
                  const SizedBox(width: 32),
                  _buildStatItem('Receipts', '${payments.length}', Colors.white),
                ],
              ),
            ],
          ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text('No payments found', style: GoogleFonts.outfit(fontSize: 18, color: theme.textTheme.bodySmall?.color)),
        ],
      ),
    );
  }
  ...
  Future<void> _downloadReceipt(BuildContext context, WidgetRef ref, Payment payment) async {
    ...
    try {
      ...
      final cycles = await rentRepo.getRentCyclesByTenancyAccess(payment.tenancyId, tenant.ownerId);
      ...
      final unit = await houseRepo.getUnitForTenant(tenancy.unitId, tenant.ownerId);
